...
public class MainActivity extends AppCompatActivity {
   ...
   private double taxAmount = 0.07; // tax percentage
   ...
   private ToggleButton toggle = (ToggleButton) findViewById(R.id.beforeTax); // status of the tax button
   ...
   private void calculate() {
      ...
      double beforeTax = billAmount * taxAmount;
      double beforeTaxTip = beforeTax * percent;
      double total = billAmount + tip;
      double totalNoTax = beforeTaxTip + billAmount;

      // display tip and total formatted as currency
      tipTextView.setText(currencyFormat.format(tip));
      // if statement if the beforeTax button is toggled
      toggle.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
         public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            if (isChecked) {
               // The toggle is enabled
               totalTextView.setText(currencyFormat.format(totalNoTax));
            } else {
